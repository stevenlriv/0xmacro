...
contract Airdrop is Ownable {
    ...
    function signatureClaim(bytes memory signature, address _to, uint256 _amount) external {
        ...
        require(!alreadyClaimed[msg.sender], "ALREADY_CLAIMED");
        ...
        require(recoveredSigner == signer, "INVALID_SIGNATURE_1");
        require(recoveredSigner != address(0), "INVALID_SIGNATURE_2");
    ...
    function merkleClaim(bytes32[] calldata _proof, address _to, uint256 _amount) external {
        ...
        for(uint256 i = 0; i < _proof.length; i++) {
            ...
            if(leaf > element) {
                leaf = keccak256(abi.encodePacked(element, leaf));
            }
            else if(leaf <= element) {
                leaf = keccak256(abi.encodePacked(leaf, element));
            }
        }

        require(leaf == merkleRoot, "INVALID_PROOF");
