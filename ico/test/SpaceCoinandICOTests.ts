...
describe("ICO Contract", function () {
  ...
  describe("Management", function () {
    it("Allow owner to advance phase forward", async function () {
      ...
      await expect(contract.progressPhases()).to.be.reverted;
    });

    it("Prevents non owner from setting advancing phases", async function () {
      const { contract, address_1 } = await loadFixture(deployICOContract);
      
      await expect(contract.connect(address_1).progressPhases()).to.be.reverted;
    });
    ...
    it("Emit phase advance event after a phase move forward", async function () {
      ...
      await expect(contract.progressPhases())
    ...
    it("Allow owner to set investors to the whitelist and remove them", async function () {
      ...
      //add investors
      await contract.addRemoveSeedInvestors(address_1.address);
      await contract.addRemoveSeedInvestors(address_2.address);
      ...
      expect(await contract.seedInvestors(address_1.address)).to.equal(false);
      expect(await contract.seedInvestors(address_2.address)).to.equal(false);
    });
    ...
    it("Prevent non owner to set investors to the whitelist and remove them", async function () {
      ...
      //add investors
      await expect(contract.connect(address_1).addRemoveSeedInvestors(address_1.address)).to.be.reverted;
  ...
  describe("SeedPhase", function () {
    ...
    it("Block contributions above softcap for this round", async function () {
      ...
      await contract.addRemoveSeedInvestors(address_1.address);
      await contract.addRemoveSeedInvestors(address_2.address);
      await contract.addRemoveSeedInvestors(address_3.address);
      await contract.addRemoveSeedInvestors(address_4.address);
      await contract.addRemoveSeedInvestors(address_5.address);
      await contract.addRemoveSeedInvestors(address_6.address);
      await contract.addRemoveSeedInvestors(address_7.address);
      await contract.addRemoveSeedInvestors(address_8.address);
      await contract.addRemoveSeedInvestors(address_9.address);
      await contract.addRemoveSeedInvestors(address_10.address);
      await contract.addRemoveSeedInvestors(address_11.address);
      ...
      await expect(contract.connect(address_11).contribute({value: ethers.utils.parseEther("1500")})).to.be.reverted;
    });
  });
  ...
  describe("GeneralPhase", function () {
    ...
    it("Block contributions above hardcap for this round", async function () {
      ...
      //change state
      await contract.progressPhases();//phase=1. general phase
      await contract.progressPhases();//phase=2. open phase with no limit so they can invest gaian
  ...
  describe("OpenPhase", function () {
    ...
    it("Allow to claim tokens its phase open", async function () {
      ...
      //change state
      await contract.progressPhases();//phase=2. open phase with no limit so they can invest gaian and claim tokens
    ...
    it("Prevent unearned token redemptions", async function () {
      ...
      //change state
      await contract.enableDisableICO();
      await contract.progressPhases(); //phase=1
      await contract.progressPhases(); //phase=2
